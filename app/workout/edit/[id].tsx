...
export default function EditWorkoutScreen() {
  ...
  const app = useApp();
  const workout = app.customWorkouts.find((w) => w.id === id);
  ...
  function addExercise(exName: string, muscleGroup: MuscleGroup) {
    setExercises((prev) => [...prev, { id: uid(), name: exName, muscleGroup, sets: 3, reps: 12 }]);
    setExModal(false);
    setExSearch("");
    setCustomMode(false);
    setCustomName("");
  ...
  function updateSets(exId: string, sets: number) {
    setExercises((prev) => prev.map((e) => e.id === exId ? { ...e, sets } : e));
  }

  function updateReps(exId: string, reps: number) {
    setExercises((prev) => prev.map((e) => e.id === exId ? { ...e, reps } : e));
  }
  ...
  function save() {
    ...
    if (!exercises.length)   { Alert.alert("Adicione ao menos 1 exercício"); return; }
    app.updateCustomWorkout(id as string, { name: name.trim(), exercises, splitLabel: undefined });
    router.back();
  }
  ...
  return (
    <View className="flex-1 bg-zinc-950">
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
        <View className="px-5 pt-4">
          ...
          <View className="flex-row items-center justify-between mb-3">
            ...
            <TouchableOpacity onPress={() => setExModal(true)} className="flex-row items-center gap-1">
          ...
          {exercises.length === 0 ? (
            ...
            >
              ...
              <Text className="text-zinc-500 text-sm mt-2">Nenhum exercício</Text>
            </TouchableOpacity>
          ) : (
            exercises.map((ex) => (
                ...
                <View className="flex-row gap-4">
                  <View className="flex-1">
                    ...
                    <View className="flex-row items-center gap-2">
                      <TouchableOpacity onPress={() => updateSets(ex.id, Math.max(1, ex.sets - 1))} className="w-8 h-8 bg-zinc-800 rounded-lg items-center justify-center">
                        <Ionicons name="remove" size={14} color="#a1a1aa" />
                      </TouchableOpacity>
                      <Text className="text-white font-bold w-6 text-center">{ex.sets}</Text>
                      <TouchableOpacity onPress={() => updateSets(ex.id, ex.sets + 1)} className="w-8 h-8 bg-zinc-800 rounded-lg items-center justify-center">
                  ...
                  <View className="flex-1">
                    ...
                    <View className="flex-row items-center gap-2">
                      <TouchableOpacity onPress={() => updateReps(ex.id, Math.max(1, ex.reps - 1))} className="w-8 h-8 bg-zinc-800 rounded-lg items-center justify-center">
                        <Ionicons name="remove" size={14} color="#a1a1aa" />
                      </TouchableOpacity>
                      <Text className="text-white font-bold w-6 text-center">{ex.reps}</Text>
                      <TouchableOpacity onPress={() => updateReps(ex.id, ex.reps + 1)} className="w-8 h-8 bg-zinc-800 rounded-lg items-center justify-center">
                        <Ionicons name="add" size={14} color="#a1a1aa" />
                      </TouchableOpacity>
                    </View>
                  </View>
      ...
      <View className="absolute bottom-6 left-5 right-5">
        <TouchableOpacity onPress={save} className="bg-indigo-500 h-14 rounded-2xl items-center justify-center" activeOpacity={0.85}>
          ...
          <View className="bg-zinc-900 rounded-t-3xl px-5 pt-5 pb-8" style={{ maxHeight: "80%" }}>
            <View className="flex-row items-center justify-between mb-4">
              ...
              {customMode ? (
                ...
                <TouchableOpacity onPress={() => setCustomMode(true)} className="flex-row items-center gap-1">
            ...
            {customMode ? (
              <View>
                ...
                />
                <TouchableOpacity onPress={addCustomExercise} className="bg-indigo-500 h-11 rounded-xl items-center justify-center mt-4">
